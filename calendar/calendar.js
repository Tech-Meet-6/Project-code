...
$(document).ready(function () {
    ...
    $('#add-event').click(function () {
        ...
        if(event_name.length === 0 || event_description.length === 0 || event_link.length === 0) {
            alert('Event Name, Link & Description should be present!');
            return false;
        }

        var existing_events = JSON.parse(localStorage.getItem(selected_date));
        var event_data = {'event_name': event_name, 'event_description': event_description, 'event_link': event_link};
...
function showCreateEvents() {
    ...
    if(events) {
        ...
        $.each(events, function (index, event) {
            event_lists += "<span class='index-num'>"+(index+1)+". </span>"+
            "<span class='event-name'>" + event.event_name + "</span>" +
            '<br> <a class="event-link" href="' + event.event_link + '">'+ event.event_link+'</a>' +
            "<br> <a class='event-description'>"+ event.event_description + "</a><div></div>"
