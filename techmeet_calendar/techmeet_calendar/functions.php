...
function getEvents($date = ''){ 
    ...
    global $db; 
    $result = $db->query("SELECT event_title, event_link, event_information, event_type FROM `Events` WHERE date = '".$date."' AND status = 1"); 
    if($result->num_rows > 0){ 
        ...
        while($row = $result->fetch_assoc()){ $i++; 
            ...
            <br> <a class="event-description">'.$row["event_type"]. '</a>
...
// //generate for the filtering functions
// function getFilters($event_){
//     global $db; 
//     //filtering options for user
//     $filter_option = $db->query("SELECT DISTINCT event_type FROM `events` ORDER BY event_type");
//     if($filter_option->num_rows > 0){
//         $eventlistHTML .= '<ul class="list-group">';
//         while($row=$filter_option->fetch_assoc()){
//             $eventlistHTML .= 
//             '<li class="list-group-item">
//                 <div class="form-check">
//                     <label class="form-check-label">
//                         <input type="checkbox" class="form-check-input unique_event_type" 
//                         value="'.$row['event_type'].'" id="event_type">'.$row['event_type'].'
//                     </label>
//                 </div>
//             </li>';
//         }
